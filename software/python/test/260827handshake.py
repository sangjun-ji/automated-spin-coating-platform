...
# --- 공정 시작 전 링크 안정성 시험 (기본 비활성) ---
# 아이들 상태에서 vbus만 폴링하므로, 실제 실패 모드(스핀 중 스위칭 노이즈,
# 장시간 유휴 뒤 USB selective suspend)를 재현하지 못한다. 연결 자체의 성패는
# connect_odrive()에서 이미 걸러지고, 구동 중 끊김은 keepalive/워치독/재시도
# 래퍼가 처리한다. 노이즈 문제 재발 시 진단용으로만 True로 되돌릴 것.
ODRIVE_PREFLIGHT_ENABLE = False
...
# --- 스핀 중 실시간 모니터 (공정 시간 + 실제 RPM) ---
# 어차피 대기(sleep)만 하고 있는 구간을 실시간 표시로 바꾸는 것뿐이다.
# 갠트리/피펫이 움직이는 동안에는 절대 출력하지 않으므로 터미널이 섞이지 않는다.
SPIN_MONITOR_ENABLE = True
SPIN_MONITOR_INTERVAL = 0.2        # 화면 갱신 주기(초)
SPIN_MONITOR_MIN_DURATION = 0.3    # 이보다 짧은 대기는 그냥 sleep (깜빡임 방지)

# --- 스핀 공정 타임라인 (설정값 vs 실측값 검증) ---
# 스핀 중에는 피펫/갠트리 로그가 쏟아져서 시간 정보가 묻힌다. 그래서
#  1) 스핀 중 모든 스텝 헤더에 T+경과시간을 새기고
#  2) 핵심 사건(분사/정지)은 눈에 띄는 마커로 남기고
#  3) 정지 시점에 설정값 대비 실측값 요약표를 출력한다.
SPIN_TIMELINE_ENABLE = True
SPIN_TIMELINE_TOLERANCE = 0.2      # 이 오차(초) 이내면 ✅, 넘으면 ⚠️
...
# ---------- 스핀 공정 타임라인 ----------
# 스핀 시작(T0)부터 정지까지의 사건을 기록해, 레시피에 설정한 값과
# 실제로 걸린 시간이 일치하는지 눈으로 확인할 수 있게 한다.
_spin_t0 = None          # 스핀 기동 명령을 낸 절대 시각
_spin_events = []        # [(t_rel, 라벨, 설정값 or None), ...]
_spin_plan = {}          # {"rpm":..., "spin_time":..., "accel":...}


def spin_elapsed():
    """스핀 시작 후 경과 시간(초). 스핀 중이 아니면 None."""
    if _spin_t0 is None:
        return None
    return time.time() - _spin_t0


def spin_tag():
    """스핀 중이면 스텝 헤더 뒤에 붙일 시간 태그, 아니면 빈 문자열."""
    if not SPIN_TIMELINE_ENABLE:
        return ""
    el = spin_elapsed()
    return "" if el is None else f"   ⏱️ 스핀 T+{el:.1f}s"


def spin_timeline_begin(rpm, spin_time, accel):
    """스핀 기동 시점에 호출. 타임라인을 리셋하고 T0를 잡는다."""
    global _spin_t0, _spin_events, _spin_plan
    _spin_t0 = time.time()
    _spin_events = []
    _spin_plan = {"rpm": rpm, "spin_time": spin_time, "accel": accel}
    if SPIN_TIMELINE_ENABLE:
        print(f"   ⏱️ [타이머 시작] T+0.000s - 이후 모든 스텝에 스핀 경과 시간이 표시됩니다.")


def spin_mark(label, planned=None, quiet=False):
    """
    스핀 중 핵심 사건을 기록한다. planned를 주면 설정값 대비 오차도 같이 남긴다.
    반환: 경과 시간(초) 또는 None.
    """
    el = spin_elapsed()
    if el is None:
        return None
    _spin_events.append((el, label, planned))

    if SPIN_TIMELINE_ENABLE and not quiet:
        if planned is None:
            print(f"   ┃ ⏱️ T+{el:7.3f}s ─ {label}")
        else:
            err = el - planned
            mark = "✅" if abs(err) <= SPIN_TIMELINE_TOLERANCE else "⚠️"
            print(f"   ┃ ⏱️ T+{el:7.3f}s ─ {label}  "
                  f"(설정 {planned:.1f}s / 오차 {err:+.3f}s {mark})")
    return el


def _disp_width(s):
    """터미널 표시 폭. 한글/이모지는 2칸을 차지하므로 그만큼 계산해준다."""
    import unicodedata
    w = 0
    for ch in s:
...
def _pad_to(s, width):
    """표시 폭 기준으로 오른쪽 공백을 채운다."""
    return s + " " * max(1, width - _disp_width(s))


def spin_timeline_report():
    """스핀 정지 후 호출. 설정값 대비 실측값 요약표를 출력하고 타임라인을 닫는다."""
    global _spin_t0, _spin_events, _spin_plan
    if not SPIN_TIMELINE_ENABLE or _spin_t0 is None:
        _spin_t0 = None
        return

    plan_time = _spin_plan.get("spin_time")
    rpm = _spin_plan.get("rpm")

    print("\n" + "═" * 62)
    print(f"  🌀 스핀 공정 타임라인 요약  (목표 {rpm} RPM / 설정 유지 {plan_time}s)")
    print("═" * 62)
    for t_rel, label, planned in _spin_events:
        head = f"   T+{t_rel:7.3f}s   "
        if planned is None:
            print(head + label)
        else:
            err = t_rel - planned
            mark = "✅" if abs(err) <= SPIN_TIMELINE_TOLERANCE else "⚠️"
            print(head + _pad_to(label, 30)
                  + f"설정 {planned:.1f}s / 오차 {err:+.3f}s {mark}")
    print("─" * 62)

    # 정지 명령 시점 = spin_time 검증 기준
    stop_t = next((t for t, lb, _ in _spin_events if "정지 명령" in lb), None)
    end_t = _spin_events[-1][0] if _spin_events else None

    if plan_time is not None and stop_t is not None:
        err = stop_t - plan_time
        mark = "✅" if abs(err) <= SPIN_TIMELINE_TOLERANCE else "⚠️"
        print(f"   설정 spin_time {plan_time:.1f}s → 실측 {stop_t:.3f}s "
              f"(오차 {err:+.3f}s {mark})")
    if end_t is not None:
        print(f"   총 회전 시간(감속·영점 안착 포함): {end_t:.3f}s")
    print("═" * 62 + "\n")
...
def monitor_spin_status(odrv, duration_sec, spin_start_time, label="스핀 유지"):
    """
    duration_sec 동안 '대기하면서' 공정 경과 시간과 실제 RPM을 한 줄로 갱신 출력한다.
    time.sleep(duration_sec)의 완전한 대체품 - 대기 시간은 동일하다.

    갠트리/피펫이 움직이는 구간에서는 호출하지 않는다 (출력이 섞이므로).
    스핀이 도는 동안 어차피 아무것도 안 하고 기다리는 구간에서만 쓴다.
    """
    if duration_sec <= 0:
        return

    # 모니터를 못 쓰는 상황이면 원래대로 그냥 대기
    if (not SPIN_MONITOR_ENABLE or odrv is None
            or duration_sec < SPIN_MONITOR_MIN_DURATION):
    ...
    while True:
        ...
        proc_time = (now - spin_start_time) if spin_start_time is not None else (now - start_t)

        try:
            with ODRV_LOCK:
                curr_rpm = odrv.axis0.encoder.vel_estimate * 60.0
        except Exception:
            # 통신이 잠깐 끊겨도 대기 자체는 계속되어야 한다 (타이밍 보존).
            # 복구는 keepalive 스레드가 담당한다.
            curr_rpm = float('nan')

        rpm_str = "  ----" if curr_rpm != curr_rpm else f"{curr_rpm:6.1f}"
        print(f"\r   📊 [공정 시간: {proc_time:6.1f}초] {label} | 현재 스핀코터 속도: {rpm_str} RPM",
              end="", flush=True)

        time.sleep(min(SPIN_MONITOR_INTERVAL, max(end_t - time.time(), 0.0)))

    print()  # 줄바꿈 (다음 로그가 같은 줄에 겹치지 않도록)
...
def verify_spin_speed(odrv, target_rpm, settle=1.5,
    ...
    if not ODRIVE_VERIFY_SPIN or odrv is None or target_rpm <= 0:
        return True
    # 가속 안정화 대기 = 스핀 직후 아무것도 안 하는 구간 -> 실시간 표시
    monitor_spin_status(odrv, settle, spin_start_time, label="기동 가속 중")
    try:
        with ODRV_LOCK:
...
PROCESS_RECIPE = [
    ...
    #gripper location on spincoater
    {"step": "이동", "x": 349.9, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": 81.6, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay":  0.1},
    {"step": "X/Y/Z축 이동 동작", "x": None, "y": None, "z": 69.4, "grip": None, "delay": 0.1},
    {"step": "그리퍼", "x": None, "y": None, "z": None, "grip": 675, "delay": 0.1},
    {"step": "X/Y/Z축 이동 동작", "x": None, "y": None, "z": 10, "grip": None, "delay": 0.1},     

    #진공 펌프 ON
    {"step": "진공 펌프 ON", "x": None, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": 1, "spin": None, "delay": 0.1},  
    ...
    {"step": "피펫 Z축 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp150000,120000", "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "피펫 흡입/장착", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zg50000,80", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},
    {"step": "피펫 Z축 상승", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp34000,120000", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},
    ...
    {"step": "피펫 Z축 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp64000,120000", "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    ...
    {"step": "피펫 Z축 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp115000,30000", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},
    {"step": "흡입", "x":  None, "y": None, "z":None, "grip": None, "pip_z": None, "pipette":"Ia15000,100,10", "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},
    {"step": "피펫 Z축 상승", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp34000,120000", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},

    #pipette location on spincoater
    {"step": "이동", "x": 353.5, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": 161.7, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay":  0.1},
    {"step": "피펫 Z축 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp50000,120000", "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
   
    #SPIN ON
    {"step": "스핀코터 ON", "x": None, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": 2000, "accel_time": 2.0, "spin_time": 40, "delay": 0.1},

    #perov 분사
    {"step": "분사", "x":  None, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": "Da15000,50,150,80", "wait_spin_time": 2.0, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},     
    {"step": "피펫 Z축 상승", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp34000,120000", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},

    #팁 해제 
    {"step": "이동", "x": None, "y": 20, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "해제", "x":  None, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": "Dt16000,0", "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},
        
    #2 ANTI-SOL
    #팁 장착 1-2
    {"step": "이동", "x": 494, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": 10.9, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay":  0.1},
    {"step": "피펫 Z축 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp150000,120000", "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "피펫 흡입/장착", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zg50000,80", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},
    {"step": "피펫 Z축 상승", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp34000,120000", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},
    ...
    #vial 1-2 2pacz
    {"step": "이동", "x": 682, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": 19.3, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay":  0.1},  
    {"step": "피펫 Z축 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp64000,120000", "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "감지", "x":  None, "y": None, "z":None, "grip": None, "pip_z": None, "pipette":"Ld1,10000", "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},
    {"step": "피펫 Z축 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp115000,30000", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},
    ...
    #Anti-solvent 분사
    {"step": "분사", "x":  None, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": "Da15000,50,150,80", "wait_spin_time": 30.0, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},     
    {"step": "피펫 Z축 상승", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp34000,120000", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},
    ...
    #SPIN OFF
    {"step": "스핀코터 OFF", "x": None, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": 0, "delay": 0.1},

    #gripper location on spincoater
    {"step": "이동", "x": 349.9, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": 81.3, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay":  0.1},
        
    #진공 펌프 off
    {"step": "진공 펌프 off", "x": None, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": 0, "spin": None, "delay": 0.1},  
...
if __name__ == "__main__":
    ...
    try:
        ...
        for idx, task in enumerate(PROCESS_RECIPE):
            print(f"\n▶ [STEP {idx+1}] {task['step']}{spin_tag()}")

            step_wait_target = None   # 이 스텝의 목표 분사 시점 (요약표 검증용)

            if task.get('wait_spin_time') is not None and spin_start_time is not None:
                target_wait = task['wait_spin_time']
                step_wait_target = target_wait
                ...
                if remaining > 0:
                    ...
                    # 이 구간은 스핀만 돌고 갠트리/피펫은 정지 -> 실시간 표시
                    monitor_spin_status(odrv0, remaining, spin_start_time,
                                        label=f"{target_wait:.0f}초 시점 대기")
                else:
                    print(f"   ⚠️ [정밀 타이밍] 이미 {-remaining:.2f}초 지났습니다! "
                          f"(앞 스텝이 {target_wait}초보다 오래 걸림)")
            ...
            if task.get('pipette') is not None and s_pip and s_pip.is_open:
                ...
                # 스핀 중이라면 '명령을 실제로 내보낸 순간'을 타임라인에 박아둔다.
                # 스텝 헤더 시각이 아니라 이 시각이 wait_spin_time 검증 기준이다.
                spin_mark(f"피펫 «{pip_cmd.split(',')[0]}» 전송", planned=step_wait_target)
            ...
            if task.get('spin') is not None and odrv0 is not None:
                ...
                if target_rpm > 0:
                    ...
                    spin_timeline_begin(target_rpm, target_spin_duration, accel_time_val)
                    ...
                    spin_mark(f"기동 명령 전달 완료 ({target_rpm} RPM)")
                    ...
                    spin_mark("기동 RPM 검증 통과")
                else:
                    if spin_start_time is not None:
                        elapsed = time.time() - spin_start_time
                        remaining = target_spin_duration - elapsed
                        if remaining > 0:
                            print(f"   ⏳ [스핀 유지] 총 {target_spin_duration:.1f}초 시점까지 "
                                  f"{remaining:.1f}초 남음")
                            # 정지 직전 잔여 유지 시간 -> 실시간 표시
                            monitor_spin_status(odrv0, remaining, spin_start_time,
                                                label="정지 대기")
                        else:
                            print(f"   ⚠️ [스핀 유지] 설정 {target_spin_duration:.1f}초를 "
                                  f"{-remaining:.2f}초 초과했습니다! (앞 스텝이 오래 걸림)")

                    spin_mark("정지 명령 전송", planned=target_spin_duration)
                    control_spin_cooter(odrv0, 0, recipe_start_time=spin_start_time)
                    odrv0 = ODRV if ODRV is not None else odrv0
                    spin_mark("모터 정지 + 영점 안착 완료")
                    spin_timeline_report()
                    spin_start_time = None
