...
s_spin_global = None        
s_safe_global = None
s_relay_global = None      # GUI 수동 제어를 위해 메가 보드 글로벌 변수 추가
...
PROCESS_RECIPE = [
    ...
    {"step": "진공 흡착 테스트", "pump": 1, "valve": 1, "spin": 900, "delay": 5}, # 진공이 잘 잡히는지
    {"step": "스핀 가속 테스트", "pump": 1, "valve": 1, "spin": 940, "delay": 5}, # 파워가 안 꺼지는지
    {"step": "종료 테스트", "pump": 0, "valve": 0, "spin": 900, "delay": 2}      # 안전하게 멈추는지
...
def automation_task(): 
    global safety_monitoring_active, shutdown_triggered
    global current_step_name, target_spin_rpm, target_hotplate_temp
    ...
    try:
      ...
      # 분리해서 두 개를 모두 연결
        ...
        print("\n🔒 [초기 세이프티 가드] 릴레이 강제 OFF 및 스핀코터 대기 모드 진입")
        if s_safe and s_safe.is_open:
            s_safe.write(b"P0\n"); time.sleep(0.05)
            s_safe.write(b"V0\n"); time.sleep(0.05)
            
        if s_spin and s_spin.is_open:
            s_spin.write(b"R900\n"); s_spin.flush() # 드론 모터 정지 신호 900
        ...
        try:
            ...
            if 's_relay' in locals() and s_relay is not None and s_relay.is_open: s_relay.close()
        except: pass
...
def main():
    ...
    def manual_pump_off():
        if s_relay_global and s_relay_global.is_open: s_relay_global.write(b"P0\n")
    def manual_valve_off():
        if s_relay_global and s_relay_global.is_open: s_relay_global.write(b"V0\n")
    def manual_fan_off():
        if s_spin_global and s_spin_global.is_open: s_spin_global.write(b"R900\n")
