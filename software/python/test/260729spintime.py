...
def control_spin_cooter(odrv, target_rpm):
    """
    target_rpm > 0  : 스핀코터를 지정 RPM으로 즉시 회전시키고 곧바로 통과(비동기)
    target_rpm == 0 : 스핀코터를 정지시키고 0.5도 이내 정밀 영점 안착 수행
    """
...
#1 PEROV
    ...
    #SPIN ON
    {"step": "스핀코터 ON", "x": None, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": 2000, "spin_time": 45, "delay": 0.1},
...
#2 ANTI-SOL
    ...
    # 💡 [안티솔벤트 정밀 분사] 스핀 ON 후 정확히 30초 시점에 분사하도록 타겟 설정
    {"step": "안티솔벤트 분사", "x":  None, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": "Da15000,50,150,80", "hotplate": None, "pump": None, "valve": None, "spin": None, "wait_spin_time": 30.0, "delay": 2},     
...
if __name__ == "__main__":
    ...
    try:
        ...
        for idx, task in enumerate(PROCESS_RECIPE):
            ...
            # 💡 [정밀 타이밍 제어] 스핀 시작 후 지정된 타깃 시간(예: 30초)까지 대기
