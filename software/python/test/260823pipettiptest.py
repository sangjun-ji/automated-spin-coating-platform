...
def cleanup_all_hardware():
    """프로그램 정상/비정상 종료 시 모든 COM 포트를 무조건 강제 해제하는 안전장치"""
    for s in ACTIVE_SERIALS:
        if s and hasattr(s, 'is_open') and s.is_open:
            try:
                s.close()
            except:
                pass
    print("🔌 [안전] 모든 시리얼 포트가 OS 수준에서 강제 해제되었습니다.")

atexit.register(cleanup_all_hardware)
...
# ==========================================
# 2. 안전 시리얼 포트 오픈 및 헬퍼 함수
# ==========================================
def safe_open_serial(port, baudrate, bytesize=serial.EIGHTBITS, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=1.0, write_timeout=1.0):
    """Write Timeout 및 포트 잠김 방지 패러미터가 내장된 시리얼 포트 오픈 함수"""
    try:
        ser = serial.Serial(
            port=port,
            baudrate=baudrate,
        ...
        print(f"❌ [{port}] 포트 오픈 실패 (점유 중이거나 미연결): {e}")
        return None
...
def send_pipette_oem(ser, addr, cmd_str):
    """피펫 OEM RS485 패킷 전송 (Write Timeout 및 입출력 버퍼 안전 처리)"""
    if not ser or not ser.is_open:
        print(f"⚠️ [피펫 RS485] Addr:{addr} 포트가 열려있지 않음")
        return
    ...
    try:
        ...
        print(f"❌ [피펫 RS485 (Addr:{addr})] Write Timeout 발생! (통신선/전원 점검 필요)")
    except Exception as e: 
        print(f"❌ 피펫 통신 에러: {e}")

def detect_liquid_and_stop_z(ser, p_addr=1, z_addr=41, z_move_cmd="Zp104000,20000", timeout=10.0):
    """
    명령어 ACK 응답을 비우고, 실제 액체 접촉 이벤트만 수신하여 Z축을 정지하는 함수
    """
    if not ser or not ser.is_open:
        print("❌ [액면 감지] 시리얼 포트 미연결")
        return False

    print("\n🔍 [실시간 액면 감지] 감시 모드 진입...")

    def build_oem_frame(addr, cmd_str):
        header = 0xAA
        cmd_bytes = cmd_str.encode('ascii')
        frame = bytearray([header, addr, len(cmd_bytes)]) + cmd_bytes
        checksum = sum(frame) % 256
    ...
    try:
        ...
        # 1. 피펫(Addr: 1)에 액면 감시 명령 전송
        ld_frame = build_oem_frame(p_addr, "Ld1,10000")
        ser.write(ld_frame)
        ser.flush()
        print(f"   └ 📤 [피펫 (Addr:{p_addr})] 감시 명령 전송: Ld1,10000")

        # 2. ★ [핵심] 명령어 수신 확인 응답(ACK) 버퍼 제거 ★
        time.sleep(0.15)
        if ser.in_waiting > 0:
            ack_resp = ser.read_all()
            print(f"   └ 📩 [피펫 ACK 확인 및 버퍼 제거]: {ack_resp.hex(' ').upper()}")

        # 3. Z축 모터(Addr: 41) 하강 시작 명령 전송
        z_frame = build_oem_frame(z_addr, z_move_cmd)
        ser.write(z_frame)
        ser.flush()
        print(f"   └ 📤 [Z축 (Addr:{z_addr})] 하강 개시: {z_move_cmd}")
        ...
        while (time.time() - start_time) < timeout:
            if ser.in_waiting > 0:
                ...
                print(f"   🎯 [실시간 액체 접촉 신호 수신!]: {hex_resp}")
                detected = True
                break
            time.sleep(0.01)

        # 5. Z축 정지 명령 전송
        stop_frame = build_oem_frame(z_addr, "Zs")
        ser.write(stop_frame)
        ser.flush()

        if detected:
            print("✅ [성공] 액체 접촉 확인! Z축 정지(Zs) 완료")
            return True
        else:
            print("❌ [타임아웃] 액체 미감지 -> Z축 안전 정지(Zs) 전송")
            return False

    except Exception as e:
        print(f"❌ [액면 감지 제어 에러]: {e}")
        return False
...
PROCESS_RECIPE = [
    ...
    {"step": "액체 용기 상단 대기 위치 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp64000,120000", "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 2},
    
    # 실시간 감지 및 동기화 Z축 하강 정지 공정
    {"step": "실시간 액면 감지 및 하강 정지", "detect_drop": True, "pip_z_cmd": "Zp104000,20000", "delay": 0.5},
...
if __name__ == "__main__":
    ...
    # 1. 하드웨어 시리얼 연결
    s_grip = safe_open_serial(GRIPPER_PORT, BAUD_GRIP)
    s_pip  = safe_open_serial(PIPETTE_PORT, BAUD_PIP)
    s_hotplate = safe_open_serial(
        HOTPLATE_PORT, 
        baudrate=9600, 
    ...
    s_relay = safe_open_serial(RELAY_PORT, BAUD_RELAY)

    if s_relay and s_relay.is_open:
        print("⏳ [릴레이 보드] DTR 리셋 부팅 대기 중 (1.5초)...")
        time.sleep(1.5)
        s_relay.write(b"P0\r\n")
        s_relay.flush()

    print("✅ 모든 RS-485 및 릴레이 시리얼 포트 검증 완료!")
    ...
    try:
        ...
        for idx, task in enumerate(PROCESS_RECIPE):
            ...
            # --- [3] 실시간 액면 감지 & 동기화 Z축 하강/정지 ---
            if task.get('detect_drop'):
                z_cmd = task.get('pip_z_cmd', 'Zp104000,20000')
                detect_liquid_and_stop_z(
                    ser=s_pip,
                    p_addr=P_ADDR,
                    z_addr=P_Z_ADDR,
                    z_move_cmd=z_cmd,
            ...
            # --- [4] 피펫 Z축 (Addr: 41) 일반 이동 ---
            elif task.get('pip_z') is not None and s_pip and s_pip.is_open:
                send_pipette_oem(s_pip, P_Z_ADDR, task['pip_z'])
                
            # --- [5] 피펫 본체 SADP20 (Addr: 1) 일반 제어 ---
