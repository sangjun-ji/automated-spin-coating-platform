...
# 0. 설정 파라미터
# ==========================================
TARGET_INIT_HOTPLATE_TEMP = 70.0
HOTPLATE_BOOST_OFFSET =30.0
...
def monitor_spin_status(odrv, duration_sec, recipe_start_time, interval=0.2):
    """지정된 시간 동안 첫 스핀 기준 공정 경과 시간(초)과 실제 RPM을 터미널에 실시간 출력"""
...
# ==========================================
# 3. ODrive 스핀코터 제어 함수 (안전 1초 감속 적용)
# ==========================================
def control_spin_cooter(odrv, target_rpm, accel_time_sec=1.0, recipe_start_time=None):
    ...
    if target_rpm > 0:
        ...
        # 💡 현재 속도를 측정하여 딱 1초 만에 0 RPM에 도달하도록 감속 램프 설정
        try:
...
# ==========================================
PROCESS_RECIPE = [
    # 1. 초기 설정: 핫플레이트 온도(필요시 수정) 및 진공 펌프 작동
    {"step": "핫플레이트 설정 (예: 105°C)", "hotplate": 110.0, "pump": None, "spin": None, "delay": 1.0},
    {"step": "진공 펌프 ON", "hotplate": None, "pump": 1, "spin": None, "delay": 1.0},

    {"step": "1단계 코팅 (1000 RPM, 10초 유지)", "hotplate": None, "pump": None, "spin": 4000, "accel_time": 2.0, "spin_time": 20.0, "delay": 0.0},

    
    # 4. 공정 종료: 스핀코터 정지 (1초 안전 감속) 및 진공 펌프 OFF
...
if __name__ == "__main__":
    ...
    # 4. 레시피 실행 루프
    try:
        recipe_start_time = None

        for idx, task in enumerate(PROCESS_RECIPE):
            # 첫 번째 스핀 동작(spin > 0) 시점부터 공정 시간 측정 시작
            if task.get('spin') is not None and task['spin'] > 0 and recipe_start_time is None:
                recipe_start_time = time.time()
                print("⏱️ [타이머 시작] 첫 스핀 동작 개시! 공정 시간이 0.0초부터 측정됩니다.")

            if recipe_start_time is not None:
                proc_time_now = time.time() - recipe_start_time
                time_str = f"공정 시간: {proc_time_now:.1f}s"
            else:
                time_str = "공정 시작 전"

            print(f"\n▶ [STEP {idx+1}] {task['step']} ({time_str})")
        ...
        total_elapsed = (time.time() - recipe_start_time) if recipe_start_time else 0.0
        print(f"\n🎉 레시피 공정이 완료되었습니다! (첫 스핀 이후 총 공정 소요 시간: {total_elapsed:.1f}초)")
