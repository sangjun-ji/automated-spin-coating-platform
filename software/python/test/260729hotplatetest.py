...
# 0. 공정 초기화 설정 파라미터
# ==========================================
# 💡 초기화 시 핫플레이트 최종 목표 온도 (예: 38.0°C)
TARGET_INIT_HOTPLATE_TEMP = 34.  
# 💡 급속 승온을 위한 오버슈트 오프셋 (기본 +10.0°C) -> 설정 온도: 48.0°C
...
def set_temperature(target_temp, address="01"):
    """핫플레이트 목표 온도(SV) 설정 패킷 생성 (DWR 명령어)"""
...
def get_current_temperature(ser, address="01"):
    """핫플레이트 현재 온도(PV) 읽기 함수 (DRS 명령어 & 7E1 파싱)"""
    try:
        command_body = f"{address}DRS,01,0001"  # 💡 DRD -> DRS 수정
        checksum = calculate_checksum(command_body)
        ...
        if response:
            print(f"\n   [RAW 수신]: {repr(response)}")
        else:
            print("\n   ⚠️ [RAW 수신]: 응답 없음 (Timeout)")
            return None

        # 💡 수신 형태 예시: '\x0201DRS,OK,014708\r\n'
        if "DRS,OK" in response:
            clean_resp = response.strip('\x02\x03\r\n ')
            parts = clean_resp.split(',')
            
            if len(parts) >= 3 and parts[1] == "OK":
                # '014708' 중 앞 4자리 '0147'(HEX) 추출 -> 10진수 변환 후 / 10.0
...
def wait_for_hotplate(ser, target_temp, boost_offset=10.0, timeout=1800, check_interval=2.0):
    ...
    print(f"   └ 승온 속도 향상을 위해 설정 온도를 {boost_temp:.1f}°C (+{boost_offset:.1f}°C)로 전송합니다.")
    ...
    while True:
        ...
        if curr_temp is not None:
            print(f"   ⏳ [급속 예열 중...] 실제 온도: {curr_temp:.1f}°C / 목표: {target_temp:.1f}°C (설정값: {boost_temp:.1f}°C)")
            
            # 2. 실제 온도가 목표 온도 이상인지 감시
            if curr_temp >= target_temp:
                print(f"\n✅ [핫플레이트] 목표 온도 ({target_temp:.1f}°C) 도달 확인! (현재 실제 온도: {curr_temp:.1f}°C)")
                
                # 3. 설정 온도를 목표값으로 원복
                reset_packet = set_temperature(target_temp)
                ser.write(reset_packet.encode('ascii'))
                time.sleep(0.2)
                print(f"   └ 🔄 핫플레이트 설정 온도를 {target_temp:.1f}°C로 원복 완료. 메인 공정을 출발합니다.\n")
                break
        else:
            print("   ⏳ [핫플레이트] 실제 온도 수신 대기 중 (응답 실패)...")

        if time.time() - start_time > timeout:
            print(f"\n❌ [핫플레이트] 예열 타임아웃 ({timeout}초 초과)!")
...
def control_spin_cooter(odrv, target_rpm):
    ...
    if target_rpm > 0:
        print(f"\n🌀 [스핀코터] {target_rpm} RPM 구동 시작 (비동기 회전 시작)...")
        ...
        print(f"   🚀 [스핀코터] {target_rpm} RPM으로 회전 가동 중 (다른 장비 이동 시작 가능)")
        ...
        print("   🎯 [스핀코터] 정밀 원점(0.5도 이내) 안착 보정 중...")
...
PROCESS_RECIPE = [
    ...
    #셀 파지
    {"step": "이동", "x": 165.6, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": 57, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": None, "z": 204.5, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y":  None, "z": None, "grip": 695, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.5},
    {"step": "이동", "x": None, "y":  None, "z":60, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
   
    #셀 to spin coater
    {"step": "이동", "x": 330.3, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": 83.6, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": None, "z": 206.7, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y":  None, "z": None, "grip": 800, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.5},
    {"step": "이동", "x": None, "y":  None, "z":60, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    
    #진공 펌프 ON
    {"step": "진공 펌프 ON", "x": None, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": 1, "spin": None, "delay": 2.0},

#1 PEROV
    #팁 장착
    {"step": "이동", "x": 79.4, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": 33.4, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay":  0.1},
    ...
    #용액 흡입
    {"step": "이동", "x": None, "y": 53.4, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay":  0.1},    
    {"step": "이동", "x": 144.4, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "피펫 Z축 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp64000,120000", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 2},
    ...
    {"step": "피펫 Z축 상승", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp40000,120000", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 2},       
    
    #피펫 to spin coater      
    {"step": "이동", "x": 335.8, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": 160.3, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "피펫 Z축 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp92000,120000", "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 2},           
    ...
    #SPIN ON
    {"step": "스핀코터 ON", "x": None, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": 2000, "delay": 0.1},

    #팁 해제 
    {"step": "이동", "x": None, "y": 21.3, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "해제", "x":  None, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": "Dt16000,0", "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 1},

#2 ANTI-SOL
    #팁 장착
    {"step": "이동", "x": 79.3, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": 43.4, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay":  0.1},
    {"step": "피펫 Z축 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp154000,120000", "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 2},
    {"step": "피펫 흡입/장착", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zg5000,80", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 2},
    {"step": "피펫 Z축 상승", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp34000,120000", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 2},
    ...
    #용액 흡입
    {"step": "이동", "x": None, "y": 53.4, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay":  0.1},    
    {"step": "이동", "x": 174.4, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "피펫 Z축 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp64000,120000", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 2},
    {"step": "감지", "x":  None, "y": None, "z":None, "grip": None, "pip_z": None, "pipette":"Ld1,10000", "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 0.1},
    {"step": "피펫 Z축 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp104000,20000", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 2},
    {"step": "흡입", "x":  None, "y": None, "z":None, "grip": None, "pip_z": None, "pipette":"Ia15000,100,10", "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 2.2},
    {"step": "피펫 Z축 상승", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp40000,120000", "pipette": None, "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 2},       
    #피펫 to spin coater      
    {"step": "이동", "x": 337.2, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": 160.3, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "피펫 Z축 하강", "x":  None, "y": None, "z":None, "grip": None, "pip_z": "Zp92000,120000", "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 2},           
    {"step": "분사", "x":  None, "y": None, "z":None, "grip": None, "pip_z": None, "pipette": "Da15000,50,150,80", "hotplate": None, "pump": None, "valve": None, "spin": None, "delay": 2},     
    ...
    #진공펌프 OFF
    {"step": "진공 펌프 OFF", "x": None, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": 0, "spin": None, "delay": 2.0},
      
    #스핀코터 셀 집기
    {"step": "이동", "x": 330.3, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": 83.6, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": None, "z": 207, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y":  None, "z": None, "grip": 695, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.5},
    {"step": "이동", "x": None, "y":  None, "z":60, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    
    #스핀코터 to hot plate
    {"step": "이동", "x": 545.1, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y": 6.6, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y":  None, "z":136.4, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "이동", "x": None, "y":  None, "z": None, "grip": 800, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.5},
    {"step": "이동", "x": None, "y":  None, "z":60, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 3.0},
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 💡 [핵심 수정] 템코 T50 ASCII 전용 통신 설정 (7E1: 7Bits, Even Parity, 1 StopBit)
    ...
    # 3. 핫플레이트 급속 예열 부스팅 단계 (목표: 38.0도 / 부스트 설정: 48.0도)
    if s_hotplate and s_hotplate.is_open:
        ...
        print(f"\n⚠️ 핫플레이트 시리얼 미연결로 예열 로직을 생략합니다. (목표: {TARGET_INIT_HOTPLATE_TEMP}°C)")
    ...
    try:
        ...
        for idx, task in enumerate(PROCESS_RECIPE):
            ...
            if task.get('wait_spin_time') is not None and spin_start_time is not None:
                ...
                if remaining > 0:
                    ...
                    print(f"   ⚠️ [정밀 타이밍] 준비 동작 소요시간({elapsed:.2f}초)이 목표 시간({target_wait}초)을 이미 초과했습니다!")
            ...
            if task.get('spin') is not None and odrv0 is not None:
                ...
                if target_rpm > 0:
                    ...
                    if spin_start_time is not None:
                        ...
                        if remaining > 0:
                            print(f"   ⏳ [스핀코터] 목표 시간({target_spin_duration}초) 충족을 위해 {remaining:.1f}초 추가 회전 대기...")
                            time.sleep(remaining)
                        else:
                            print(f"   ⏱️ [스핀코터] 이미 {elapsed:.1f}초 경과 (목표 {target_spin_duration}초 충족) -> 즉시 정지")
        ...
        if s_hotplate and s_hotplate.is_open:
            try:
                # 안전을 위해 종료 시 핫플레이트 히터를 0도로 전환하여 끔
