...
# 4. ODrive 스핀코터 제어 함수
# ==========================================
def run_spin_cooter(odrv, target_rpm, spin_time_sec):
    """
    ODrive 스핀코터 통합 구동 함수
    - 1단계: 지정 RPM 및 시간 구동
    - 2단계: 초속 정지 후 1초 이내 정밀 원점(0.5도) 복귀
    """
    print(f"\n🌀 [스핀코터] {target_rpm} RPM 구동 시작 (목표 시간: {spin_time_sec}초)...")
    ...
    # 구동 시작 전 시작 위치 기록
    start_pos = odrv.axis0.encoder.pos_estimate
    ...
    odrv.axis0.controller.config.vel_ramp_rate = 100.0  # 고속 가/감속
    ...
    while (time.time() - s_time) < spin_time_sec:
        cur_vel = odrv.axis0.encoder.vel_estimate
        print(f"   🌀 [스핀코터] 구동 중... 현재 속도: {cur_vel*60:.0f} RPM", end="\r")
        time.sleep(0.5)
    print("")

    # 고속 감속 정지
    print("   ⏹️ [스핀코터] 목표 시간 완료! 급감속 정지 중...")
    odrv.axis0.controller.input_vel = 0.0
    while abs(odrv.axis0.encoder.vel_estimate) > 0.5:
        time.sleep(0.02)

    # 🎯 [2단계] 초정밀 원점 안착
    print("   🎯 [스핀코터] 정밀 원점(0.5도 이내) 안착 보정 시작...")
    stage2_start = time.time()
    current_pos = odrv.axis0.encoder.pos_estimate
    relative_turns = current_pos - start_pos

    nearest_target_turns = round(relative_turns)
    target_pos = start_pos + nearest_target_turns
    ...
    final_deg = ((odrv.axis0.encoder.pos_estimate - start_pos) % 1.0) * 360.0
...
# ==========================================
# 5. 통합 프로세스 레시피 (spin 키 적용)
# ==========================================
PROCESS_RECIPE = [
    {"step": "초기화 확인 및 펌프 대기", "x": None, "y": None, "z": None, "grip": 1000, "pip_z": None, "pipette": "It16000,100,2", "hotplate": 26, "pump": 0, "spin": None, "delay": 0.1},
    
    {"step": "진공 펌프 ON", "x": None, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": 1, "spin": None, "delay": 2.0},

    # 🌀 [스핀 코팅 공정 예시 스텝] 5000 RPM으로 30초 구동 후 자동 원점 안착
    {"step": "스핀 코팅 공정 수행", "x": None, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": {"rpm": 2000, "time": 10}, "delay": 0.5},
   
    {"step": "진공 펌프 OFF", "x": None, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": 0, "spin": None, "delay": 3.0},
    {"step": "X축 이동", "x": 20.0, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "Y축 이동", "x": None, "y": 32.5, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "X축 이동", "x": 220.0, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "Y축 이동", "x": None, "y": 132.5, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": None, "pump": None, "spin": None, "delay": 0.1},
    {"step": "핫플레이트 OFF", "x": None, "y": None, "z": None, "grip": None, "pip_z": None, "pipette": None, "hotplate": 0, "pump": None, "spin": None, "delay": 0.1},
]
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 💡 [개선] 엔코더가 준비되지 않은 상태(전원 갓 켜짐)라면 자동으로 캘리브레이션 수행
        if not odrv0.axis0.encoder.is_ready:
            ...
            # 캘리브레이션 완료 대기
            while odrv0.axis0.current_state != 1:  # IDLE 모드 대기
                time.sleep(0.1)
            time.sleep(0.5)
    ...
    try:
        for idx, task in enumerate(PROCESS_RECIPE):
            ...
            if task.get('spin') is not None and odrv0 is not None:
                ...
                spin_time = spin_cfg.get('time', 10)
                run_spin_cooter(odrv0, target_rpm=rpm, spin_time_sec=spin_time)
