...
PROCESS_RECIPE = [
    ...
    {"step": "이동", "x": None, "y": None, "z": 206, "grip": None,  "pip_z": None, "pipette": None,  "delay": 0.5 }, 
    {"step": "25mm 유리 기판 파지", "x": None, "y": None, "z": None, "grip": 680, "pip_z": None, "pipette": None, "delay": 2.0},
    {"step": "이동", "x": None, "y": None, "z": -3, "grip": None,  "pip_z": None, "pipette": None,  "delay": 0.5 }, 
    ...
    {"step": "이동", "x": None, "y": None, "z": 198, "grip": None,  "pip_z": None, "pipette": None,  "delay": 0.5 }, 
    ...
    while True:
        ...
        if s_safe.in_waiting > 0:
            try:
                ...
                # 💡 [진단 기능] COM14에서 문자가 한 톨이라도 오면 무조건 터미널에 띄웁니다!
                # 만약 센서를 눌렀는데 이 로그가 안 뜬다면, 아두이노 COM14 보드 세팅(하드웨어) 문제입니다.
                if raw_data.strip():
...
def wait_until_idle(ser):
    ...
    while True:
        ...
        try:
            ...
            # 💡 [핵심 추가] 공정 도중 GRBL 센서가 타격되면, 상태 메시지에 ALARM이 뜹니다.
...
def run_xyz_homing_hybrid(s):
    ...
    print("\n▶ [3/3] Y축 센서 탐색 중...")
    s.write(b'\x18'); time.sleep(2.0); s.write(b'$X\n'); time.sleep(0.5)
    s.write(b'$21=1\n'); time.sleep(0.5); s.reset_input_buffer()
    ...
    s.write(b'$131=170.000\n'); time.sleep(0.1) 
    s.write(b'$132=210.000\n'); time.sleep(0.1) 
    s.write(b'$20=1\n'); time.sleep(0.1)        
    
    # 💡 [핵심 부활!!] 기존 GRBL 센서 3개를 공정 중에도 쓸 수 있도록 하드웨어 리밋을 켭니다!
    s.write(b'$21=1\n'); time.sleep(0.1)
